...
public class Database {
	...
	public void insertTemperature(Comment comment) {
		EntityManager em = null;
		try {
			em = emf.createEntityManager();
			em.getTransaction().begin();
	...
	public Comment selectComment(int idComment) {
		Comment comment = null;
		EntityManager em = null;
		try {
			em = emf.createEntityManager();
			em.getTransaction().begin();
			comment = em.find(Comment.class, idComment);
			if (comment == null) {
				System.out.println("There's no comment on Comment");
			}
		} catch (Exception e) {
			System.err.println(e.getMessage());
		} finally {
